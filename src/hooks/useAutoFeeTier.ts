...
export function useAutoFeeTier({
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (preferBestQuote) {
          ...
          const successes = quotes
            .filter((r): r is PromiseFulfilledResult<{ fee: FeeTier; out: bigint }> => r.status === 'fulfilled')
            .map((r) => r.value)

          if (successes.length) {
            successes.sort((a, b) => (a.out > b.out ? -1 : a.out < b.out ? 1 : 0))
    ...
    run()
    return () => { active = false }
  }, [ready, client, tokenIn, tokenOut, amountInHuman, decimalsIn, preferBestQuote])
