...
export async function GET(
  ...
  // -----------------------------
  // OHLCV proxy mode:
  // /api/gecko/pool/:pool?timeframe=day&aggregate=1&limit=100...
  // (backwards compatible with old `ohlcv` param)
  // -----------------------------
  const ohlcvTf =
    searchParams.get("timeframe") ?? searchParams.get("ohlcv"); // day|hour|minute|second
  ...
  if (ohlcvTf) {
    ...
    const ohlcvQs = new URLSearchParams({
      ...
      include_empty_intervals:
        searchParams.get("include_empty_intervals") ?? "false",
