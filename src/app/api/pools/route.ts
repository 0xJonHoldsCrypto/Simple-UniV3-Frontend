...
async function multicallChunked<
  T extends { address: Address; abi: any; functionName: any; args?: any[] }
>(client: any, contracts: T[], chunkSize = 150) {
  const out: any[] = [];
  for (let i = 0; i < contracts.length; i += chunkSize) {
    ...
    try {
      ...
      res = Array.isArray(r)
        ...
        : [];
      okShare =
        res.filter((x: any) => x && x.status === "success").length /
        (res.length || 1);
    } catch {
...
export async function GET(req: Request) {
  try {
    const FACTORY = (
      process.env.NEXT_PUBLIC_UNI_FACTORY || ""
    ...
    if (!FACTORY || !RPC || !CHAIN_ID) {
      return NextResponse.json(
        {
          error:
            "Missing envs: NEXT_PUBLIC_UNI_FACTORY / NEXT_PUBLIC_RPC_URL / NEXT_PUBLIC_CHAIN_ID",
        },
        { status: 500 }
      );
    }
    ...
    const list = (tokenlist.tokens as Token[]).filter(
      (t) => t.chainId === CHAIN_ID
    );
    if (!list.length) {
      return NextResponse.json(
        { error: `No tokens for chainId ${CHAIN_ID} in tokenlist`, count: 0 },
        { status: 400 }
      );
    }
    ...
    if (!listCapped.length) {
      return NextResponse.json(
        { error: `No tokens for chain ${CHAIN_ID} in tokenlist` },
        { status: 400 }
      );
    }
    ...
    async function diagKnownPair() {
      try {
        ...
        const bySym = new Map(
          listCapped.map((t) => [t.symbol.toLowerCase(), t])
        );
        const weth = bySym.get("weth");
        const usdc = bySym.get("usdc.e") ?? bySym.get("usdc");
        if (!weth || !usdc)
          return { note: "No WETH/USDC in tokenlist for this chain." };
    ...
    if (!found.length) {
      ...
      const pairsSmall = pairsCapped.slice(
        0,
        Math.min(pairsCapped.length, 1500)
      );
    ...
    for (let i = 0; i < found.length; i++) {
      ...
      const liq =
        typeof liqRaw === "bigint"
          ? liqRaw
          : Array.isArray(liqRaw)
          ? (liqRaw[0] as bigint)
          : undefined;
      ...
      rows.push({
        ...
        slot0: s0
          ? {
              sqrtPriceX96: String(s0[0] as bigint),
              tick: Number(s0[1] as number),
            }
          : null,
      });
    }
    ...
    const byAddr = new Map(
      addrs.map((a) => [
        a,
        listCapped.find((t) => t.address.toLowerCase() === a)!,
      ])
    );
    ...
    if (debug) {
      return NextResponse.json({
        tokens: listCapped.length,
        pairsTried: pairsCapped.length,
        poolsFound: withMeta.length,
        sample: withMeta.slice(0, 5),
      });
    }
