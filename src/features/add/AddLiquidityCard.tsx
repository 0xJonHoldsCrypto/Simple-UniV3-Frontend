...
export default function AddLiquidityCard() {
  ...
  const [rangePreset, setRangePreset] = useState<"full" | "50" | "20" | "10">(
    "full"
  );
  ...
  function priceFromTick(tick: number, dec0: number, dec1: number): number {
    ...
    if (!Number.isFinite(t) || !Number.isFinite(d0) || !Number.isFinite(d1))
      return NaN;
      ...
      try {
        ...
        if (!Number.isFinite(curTickNum) && rawSqrt != null) {
          try {
            const [token0, token1] = sortTokens(tokenA, tokenB);
            const meta0 =
              token0.toLowerCase() === tokenA.toLowerCase() ? metaA : metaB;
            const meta1 =
              token1.toLowerCase() === tokenA.toLowerCase() ? metaA : metaB;
            const dec0 = Number(meta0?.decimals ?? 18);
            const dec1 = Number(meta1?.decimals ?? 18);
        ...
        if (!Number.isFinite(curTickNum)) {
          console.warn("[AddLiquidity] non-numeric tick data", {
            rawTick,
            rawSqrt,
            state,
          });
        ...
        const { tickLower, tickUpper } = ticksFromPreset(
          curTickNum,
          spacingNum,
          rangePreset
        );
        ...
        if (metaA && metaB) {
          ...
          if (Number.isFinite(prices.currentPrice))
            setCurrentPrice(prices.currentPrice);
          else setCurrentPrice(null);
          if (
            Number.isFinite(prices.minPrice) &&
            Number.isFinite(prices.maxPrice)
          ) {
  ...
  useEffect(() => {
    ...
    if (currentTick == null || tickSpacing == null || !tokenA || !tokenB)
      return;
  ...
  const derivedCurrentPrice = useMemo<number | null>(() => {
    if (!tokenA || !tokenB || !metaA || !metaB || currentTick == null)
      return null;
  ...
  const displayPrices = useMemo(() => {
    ...
    if (currentPrice == null || !Number.isFinite(currentPrice)) {
      return {
        ...
        unit:
          metaB.symbol && metaA.symbol ? `${metaB.symbol}/${metaA.symbol}` : "",
      };
    }
    ...
    if (invertPrice && cur != null && Number.isFinite(cur) && cur !== 0) {
      const invCur = 1 / cur;
      const invMin =
        maxOrdered != null && Number.isFinite(maxOrdered) && maxOrdered !== 0
          ? 1 / maxOrdered
          : null;
      const invMax =
        minOrdered != null && Number.isFinite(minOrdered) && minOrdered !== 0
          ? 1 / minOrdered
          : null;
      const invUnit =
    ...
    <div className="max-w-2xl mx-auto rounded-2xl p-4 bg-neutral-900 shadow space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between text-xs opacity-70">
              <span>Amount A {metaA ? `(${metaA.symbol})` : ""}</span>
              <span>
                Balance:{" "}
                {balA
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between text-xs opacity-70">
              <span>Amount B {metaB ? `(${metaB.symbol})` : ""}</span>
              <span>
                Balance:{" "}
                {balB
        ...
        <div className="text-[11px] opacity-70">
          {displayPrices?.cur != null && metaA && metaB ? (
            <>
              <div className="flex items-center gap-2">
                <div>
                  Current price: {formatPrice(displayPrices.cur, 4)}{" "}
                  {displayPrices.unit}
                </div>
                <button
                  type="button"
              ...
              {rangePreset === "full" && !manualRange ? (
                ...
                <div>
                  Min/Max: {formatPrice(displayPrices.min, 4)} →{" "}
                  {formatPrice(displayPrices.max, 4)} {displayPrices.unit}
                </div>
              )}
            </>
          ) : (
          ...
          <span>
            {rangePreset === "full" && !manualRange
              ...
              : displayPrices?.min != null &&
                ...
                metaA &&
                metaB
              ? `${formatPrice(displayPrices.min, 4)} → ${formatPrice(
                  displayPrices.max,
                  4
                )} ${displayPrices.unit}`
              : tickLower != null && tickUpper != null
